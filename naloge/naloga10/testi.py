...
def hitrejsi(kol1, kol2, raz):
    return kol1 if dokazov(kol1, kol2, raz)\
        else kol2 if dokazov(kol2, kol1, raz)\
        else None


def dokazov(kol1, kol2, raz):
    return int(kol2 in raz[kol1]) + \
        sum((dokazov(kol, kol2, raz)
             for kol in raz[kol1]))
