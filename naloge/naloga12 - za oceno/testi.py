...
class Kolesar:
    ...
    def pojdi(self, smer):
        if self.ziv:
            px = self.x
            py = self.y
            self.x, self.y = {"<": (self.x - 1, self.y), ">": (self.x + 1, self.y),
                              "^": (self.x, self.y - 1), "v": (self.x, self.y + 1)}[smer]
            mx = len(self.zemljevid[0]) - 1
            my = len(self.zemljevid) - 1
            if self.x < 0 or self.x > mx or self.y < 0 or self.y > my:
                self.ziv = False
                self.x = px
                self.y = py
            else:
    ...
    def prevozi(self, pot):
        if self.ziv:
            ...
            for korak in pot:
                if korak in "<>^v":
                    for _ in range(stev + (not bool(stev))):
                        self.pojdi(korak)
                    stev = 0
                else:
                    stev = stev * 10 + int(korak)
...
class Zbiralec(Kolesar):
    ...
    def pojdi(self, smer):
        super().pojdi(smer)
        if self.ziv:
            z = self.zemljevid[self.y][self.x]
            if z != ".":
                if z not in self.znacke_:
                    self.znacke_[z] = 0
                self.znacke_[z] += 1

    def znacke(self):
        return set(self.znacke_.keys())

    def naj_znacke(self):
        maks = 0
        naj = set()
        for znacka, kolicina in self.znacke_.items():
            if kolicina > maks:
                naj = set() | {znacka}
                maks = kolicina
            elif kolicina == maks:
    ...
    def trofeje(self):
        naj = {}
        for znacka, kolicina in self.znacke_.items():
            if kolicina not in naj:
                naj[kolicina] = []
            naj[kolicina].append((znacka, kolicina))
        naj_urejeni = []
        for kolicina, znacka in naj.items():
            naj_urejeni.append(znacka)
        naj_urejeni.sort(key=lambda x: (x[0][1]), reverse=True)
        naj = []
        for sezn in naj_urejeni:
            naj.extend(sorted(sezn, key=lambda x: x[0]))
            if len(naj) >= 3:
                break
        return naj
...
class Drsalec(Zbiralec):
    ...
    def pojdi(self, smer, drsenje=0):
        px, py = self.x, self.y
        super().pojdi(smer)
        if self.ziv:
            self.sledi[(px, py)] = smer
            for _ in range(3):
                if (self.x, self.y) in self.sledi:
