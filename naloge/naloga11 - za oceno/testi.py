...
def preberi_zemljevid(ime_datoteke):
    ...
    slovar = {}
    slovar_1 = {}
    for y, vrstica in enumerate(zemljevid):
        for x, znak in enumerate(vrstica):
            if znak != ".":
                ...
                slovar[znak].add((x, y))
                slovar_1[(x, y)] = znak
    return slovar, slovar_1, (len(zemljevid[0]), len(zemljevid))


def najblizji(x, y, c, zemljevid, prepovedani):
    zemljevid = zemljevid[0]
    prepovedani = prepovedani | {(x, y)}
    naj_razdalja = None
    naj_tocka = None
    kandidati = c if c else zemljevid.keys()
    for tocka in kandidati:
        for x1, y1 in zemljevid[tocka]:
            if (x1, y1) in prepovedani:
                continue
            if naj_razdalja is None or \
                    naj_razdalja > abs(x - x1) + abs(y - y1) or \
                    naj_razdalja == abs(x - x1) + abs(y - y1) and x1 < naj_tocka[0] or \
                    naj_razdalja == abs(x - x1) + abs(y - y1) and x1 == naj_tocka[0] and y1 < naj_tocka[1]:
                naj_razdalja = abs(x - x1) + abs(y - y1)
                naj_tocka = (x1, y1)
    return naj_tocka
...
def vsi_najpogostejsi(x, y, d, zemljevid):
    ...
    slovar = {}
    naj = 0
    for tocka, mnk in zemljevid.items():
        for (x1, y1) in mnk:
            if d >= abs(x - x1) + abs(y - y1):
                if tocka not in slovar:
                    slovar[tocka] = 0
                slovar[tocka] += 1
                naj = max(naj, slovar[tocka])
    pogoste = set()
    for tocka, kolicina in slovar.items():
        if kolicina == naj:
            pogoste.add(tocka)
    return pogoste
...
def najpogostejsi(x, y, d, zemljevid):
    vsi = vsi_najpogostejsi(x, y, d, zemljevid)
    if not vsi:
        return None
    return min(vsi)
...
def najboljsa_cetrt(a, zemljevid):
    ...
    naj_znam = None
    stolpci = {}
    for y in range(maks_y - a + 1):
        for x in range(maks_x - a + 1):
            znam = 0
            for x1 in range(x, x + a):
                if (x1, y) not in stolpci:
                    stolpci[(x1, y)] = sum([(x1, y1) in zemljevid for y1 in range(y, y + a)])
                znam += stolpci[(x1, y)]
            if naj_znam is None or naj_znam < znam or \
                    naj_znam == znam and x < naj_cetrt[0] or \
                    naj_znam == znam and x == naj_cetrt[0] and y < naj_cetrt[1]:
                naj_znam = znam
                naj_cetrt = (x, y)
    return naj_cetrt
